...
export function useBastionProgram() {
  ...
  const getAuditStateAddress = useCallback(() => {
    return PublicKey.findProgramAddressSync([Buffer.from(AUDIT_SEED)], programId)[0];
  }, [programId]);

  const getPolicyAddress = useCallback(() => {
    return PublicKey.findProgramAddressSync([Buffer.from(POLICY_SEED)], programId)[0];
  }, [programId]);

  const getAuditEntryAddress = useCallback((index: number) => {
    const buf = new BN(index).toArrayLike(Buffer, 'le', 8);
    return PublicKey.findProgramAddressSync([Buffer.from(AUDIT_SEED), buf], programId)[0];
  }, [programId]);
  ...
  const fetchStats = useCallback(async (): Promise<StatsData | null> => {
    ...
    try {
      ...
      return { total: Number(account.totalAudits), allowed: Number(account.allowedCount), blocked: Number(account.blockedCount) };
    } catch { return null; }
  ...
  const fetchAuditEntries = useCallback(async (limit = 50): Promise<AuditEntryData[]> => {
    if (!program) return [];
    try {
      ...
      for (let i = total - 1; i >= start; i--) {
        try {
          ...
          entries.push({
            id: i.toString(), timestamp: Number(entry.timestamp),
  ...
  const fetchPolicy = useCallback(async (): Promise<PolicyData | null> => {
    ...
    try {
      ...
      return {
        ...
        allowedPrograms: (account.allowedPrograms as any[]).map((p: any) => new PublicKey(p).toBase58()),
      };
    } catch { return null; }
  ...
  const fetchStake = useCallback(async (authority: PublicKey): Promise<any | null> => {
    ...
    try {
      const [agentStake] = PublicKey.findProgramAddressSync(
        [Buffer.from("agent_stake"), authority.toBuffer()], programId,
      );
  ...
  const emergencyPause = useCallback(async (): Promise<string | null> => {
    if (!program) return null;
    try {
      const sig = await (program.methods as any).emergencyPause().accounts({ auditState: getAuditStateAddress() }).rpc();
  ...
  const emergencyResume = useCallback(async (): Promise<string | null> => {
    if (!program) return null;
    try {
      const sig = await (program.methods as any).emergencyResume().accounts({ auditState: getAuditStateAddress() }).rpc();
      return sig as string;
    } catch { return null; }
  }, [program, getAuditStateAddress]);

  const updatePolicy = useCallback(async (
    allowedPrograms: string[], maxSolPerTx: number, rateLimitPerMinute: number,
  ): Promise<string | null> => {
    if (!program) return null;
    try {
      ...
      const sig = await (program.methods as any)
        ...
        .accounts({ policy: getPolicyAddress(), signer: wallet.publicKey!, systemProgram: PublicKey.default })
        .rpc();
      return sig as string;
    } catch { return null; }
  ...
  return useMemo(() => ({
    program, fetchStats, fetchPaused, fetchAuditEntries, fetchPolicy,
    fetchAgents, fetchStake, fetchAllAudits, emergencyPause, emergencyResume, updatePolicy,
